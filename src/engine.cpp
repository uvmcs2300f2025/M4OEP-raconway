...
state screen = start;  // Initialize to start
...
void Engine::processInput() {
    ...
    if (screen == play) {
        ...
        if (keys[GLFW_KEY_UP]) {
            spawnButton->moveY(moveSpeed);
            if (spawnButton->getTop() > height) {
                spawnButton->setPosY(height - spawnButton->getSize().y / 2);
            }
        }
        if (keys[GLFW_KEY_DOWN]) {
            spawnButton->moveY(-moveSpeed);
            if (spawnButton->getBottom() < 0) {
                spawnButton->setPosY(spawnButton->getSize().y / 2);
            }
        }
        if (keys[GLFW_KEY_LEFT]) {
            spawnButton->moveX(-moveSpeed);
            if (spawnButton->getLeft() < 0) {
                spawnButton->setPosX(spawnButton->getSize().x / 2);
            }
        }
        if (keys[GLFW_KEY_RIGHT]) {
            spawnButton->moveX(moveSpeed);
            if (spawnButton->getRight() > width) {
                spawnButton->setPosX(width - spawnButton->getSize().x / 2);
            }
        }
    }
    ...
    if (screen == play) {
        ...
        // Button was released
        if (mousePressedLastFrame && !mousePressed && buttonOverlapsMouse) {
            spawnConfetti();
        }
    }
...
void Engine::render() {
    ...
    switch (screen) {
        ...
        case play: {
            // Draw all confetti pieces first
            for (const unique_ptr<Shape>& piece : confetti) {
                piece->setUniforms();
                piece->draw();
            }

            // Draw spawn button on top
        ...
        case over: {
            ...
            // Display the message centered on the screen
            this->fontRenderer->renderText(message, width/2 - (12 * message.length()), height/2, projection, 1, vec3{1, 1, 1});
            break;
        }
    }
...
void Engine::spawnConfetti() {
    ...
    float confettiSize = confetti.size() + 1;
    vec2 size = {confettiSize, confettiSize};
